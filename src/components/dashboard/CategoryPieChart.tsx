...
import { PieChart as PieIcon, X, TrendingDown, TrendingUp } from 'lucide-react';
...
export const CategoryPieChart: React.FC = () => {
  ...
  const activeSummaries =
    chartType === 'income' ? stats.incomeCategorySummaries : stats.categorySummaries;

  const data = activeSummaries.map((item) => ({
  ...
  const isIncomeMode = chartType === 'income';
  const totalAmount = isIncomeMode ? stats.totalIncome : stats.totalExpenses;
  const totalCount = isIncomeMode ? stats.incomeCount : stats.expenseCount;

  const selectedIndex =
    filter.categoryId !== 'all' ? data.findIndex((d) => d.id === filter.categoryId) : -1;

  const displayActiveIndex =
    hoveredIndex !== null ? hoveredIndex : selectedIndex >= 0 ? selectedIndex : null;
      ...
      return (
        <div className="bg-slate-900/95 dark:bg-slate-800/95 text-white px-3 py-2 rounded-xl shadow-2xl border border-slate-700/80 text-xs backdrop-blur-md pointer-events-none min-w-[140px] transition-all duration-150">
          <div className="flex items-center justify-between gap-2 mb-1">
            ...
            {isSelected && (
              <span className="text-[8px] uppercase font-extrabold bg-emerald-500/80 text-white px-1 py-0.2 rounded">
                Filtrado
              </span>
            )}
          </div>
  ...
  const activeCategory =
    displayActiveIndex !== null && displayActiveIndex >= 0 && displayActiveIndex < data.length
  ...
  return (
    <div className="bg-white dark:bg-slate-900 p-4 sm:p-6 rounded-2xl border border-slate-200/80 dark:border-slate-800 shadow-sm flex flex-col h-full">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-3 sm:mb-4">
        <div>
          <h3 className="text-sm sm:text-base font-bold text-slate-900 dark:text-white flex items-center gap-2">
            <PieIcon className="w-4 h-4 sm:w-5 sm:h-5 text-emerald-600 dark:text-emerald-400" />
            {isIncomeMode ? 'Receitas por Categoria' : 'Despesas por Categoria'}
          </h3>
          <p className="text-[11px] sm:text-xs text-slate-500 dark:text-slate-400 mt-0.5">
            {isIncomeMode ? 'Proporção de entradas por fonte' : 'Proporção de despesas nas categorias'}
          </p>
        </div>

        <div className="flex items-center gap-2 self-start sm:self-auto">
          {/* Toggle Despesas vs Receitas */}
          <div className="flex items-center p-0.5 bg-slate-100 dark:bg-slate-800 rounded-xl border border-slate-200/60 dark:border-slate-700/60 text-xs font-semibold">
            <button
              type="button"
              onClick={() => {
          ...
          {isFilterActive && (
            <button
              ...
              className="flex items-center gap-1 px-2 py-1 rounded-lg text-xs font-semibold text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-950/60 hover:bg-emerald-100 dark:hover:bg-emerald-900/60 transition-colors"
            >
      ...
      {data.length === 0 ? (
        <div className="flex-1 flex flex-col justify-center items-center text-center py-8 min-h-[200px]">
          <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-2xl bg-emerald-50 dark:bg-emerald-950/50 flex items-center justify-center text-emerald-500 mb-2.5 sm:mb-3">
            <PieIcon className="w-5 h-5 sm:w-6 sm:h-6" />
          </div>
          <h4 className="font-semibold text-slate-800 dark:text-slate-200 text-sm">
            {isIncomeMode ? 'Nenhuma receita registrada' : 'Nenhuma despesa registrada'}
          </h4>
          <p className="text-xs text-slate-400 mt-1 max-w-xs">
            {isIncomeMode
              ? 'Cadastre entradas financeiras para visualizar o gráfico de fontes de renda.'
              : 'Cadastre gastos neste mês para visualizar o gráfico de categorias.'}
          </p>
        </div>
      ) : (
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 sm:gap-6 items-center flex-1">
              ...
              {activeCategory ? (
                <div className="animate-in fade-in zoom-in-90 duration-200 flex flex-col items-center max-w-[100px] sm:max-w-[110px]">
                  {filter.categoryId === activeCategory.id && (
                    <span className="text-[8px] uppercase font-bold tracking-wider text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-950/60 px-1.5 py-0.2 rounded-full mb-0.5">
                      Filtrado
                    </span>
                  )}
                ...
                <div className="animate-in fade-in duration-200 flex flex-col items-center">
                  <span className="text-[10px] sm:text-[11px] font-semibold text-slate-400 uppercase tracking-wider">
                    {isIncomeMode ? 'Total Entradas' : 'Total Saídas'}
                  </span>
                  <span className="text-xs sm:text-base font-black text-slate-900 dark:text-white leading-tight">
          ...
          <div className="lg:col-span-7 space-y-2 max-h-52 sm:max-h-64 overflow-y-auto pr-1 custom-scrollbar">
            {data.map((catSummary, index) => {
              const isCategorySelected = filter.categoryId === catSummary.id;
              ...
              return (
                <div
                  ...
                  onMouseLeave={() => setHoveredIndex(null)}
                  onClick={() => handleToggleCategory(catSummary.id)}
                  className={`p-2 sm:p-2.5 rounded-xl border transition-all duration-200 cursor-pointer active:scale-[0.99] ${
                    isCategorySelected
                      ? 'bg-emerald-50/90 dark:bg-emerald-950/70 border-emerald-500 ring-2 ring-emerald-500/20 shadow-sm'
                      : isHovered
                ...
                >
                  <div className="flex items-center justify-between gap-2 mb-1 sm:mb-1.5">
                    <div className="flex items-center gap-2 min-w-0">
                      ...
                      >
                        {renderCategoryIcon(catSummary.icon, 'w-3 h-3 sm:w-3.5 sm:h-3.5')}
                      </div>
                      <span className="text-xs font-semibold text-slate-800 dark:text-slate-200 truncate">
                        {catSummary.name}
                      </span>
                      <span className="text-[10px] text-slate-400">({catSummary.count})</span>
                    </div>

                    <div className="flex items-center gap-1.5 sm:gap-2 flex-shrink-0">
                      <span className="text-xs font-bold text-slate-900 dark:text-white">
                        {formatCurrency(catSummary.value)}
                      </span>
